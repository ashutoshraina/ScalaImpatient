...
object FunSets {
   ...
   */
  type Set = Int => Boolean;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(379); 

  /**
   * Indicates whether a set contains a given element.
   */
  def contains(s: Set, elem: Int): Boolean = s(elem);System.out.println("""contains: (s: Int => Boolean, elem: Int)Boolean""");$skip(120); 

  /**
   * Returns the set of the one given element.
   */
  def singletonSet(elem: Int): Set = {x : Int => elem == x};System.out.println("""singletonSet: (elem: Int)Int => Boolean""");$skip(159); 

  /**
   * Returns the union of the two given sets,
   * the sets of all elements that are in either `s` or `t`.
   */
  def union(s: Set, t: Set): Set = ???;System.out.println("""union: (s: Int => Boolean, t: Int => Boolean)Int => Boolean""");$skip(168); 
   ...
   */
  val bound = 1000;System.out.println("""bound  : Int = """ + $show(bound ));$skip(160); 
